    ...
    def setup_standard_ui(self, title, description, bg_color, media_file):
        ...
        if not self.hide_title:
            ...
            if self.large_text:
                ...
                if self.use_picture:
                    t_label.setStyleSheet("color: #2C4C49; padding: 15px;")
                else:
                    t_label.setStyleSheet("color: #2C4C49; padding-top: 10px;")
            else:
                t_label.setFont(QFont("Arial", 16, QFont.Weight.Bold))
                if self.use_picture:
                    t_label.setStyleSheet("padding: 10px;")
        ...
        if self.use_picture and media_file and os.path.exists(media_file):
            ...
            if media_file and os.path.exists(media_file):
                ...
                emoji_map = {
                    ...
                    "FOOD": "🥪", "FEELING": "😊", "EXERCISE": "💪", "ENTERTAINMENT": "📺",
                    "YES / NO": "✅", "RECOMMEND ANSWER": "💡", "CLOCK": "⏰"
                }
                icon_text = emoji_map.get(title, "✨")
                self.media_display.setText(icon_text)
        ...
        if self.large_text or self.hide_title:
        ...
        if self.hide_title:
            self.main_v_layout.addWidget(self.top_half, 75) 
            self.main_v_layout.addWidget(self.bottom_half, 25)
        elif self.large_text:
            self.main_v_layout.addWidget(self.top_half, 60) 
            self.main_v_layout.addWidget(self.bottom_half, 40)
        else:
            self.main_v_layout.addWidget(self.top_half, 40) 
            self.main_v_layout.addWidget(self.bottom_half, 60)
...
class BathRoom(QFrame):
    ...
    def create_bathroom_page(self):
        ...
        for t, d, c, media_file, r, col in options:
            card = CommBox(title=t, description=d, bg_color=c, media_file=media_file, show_btn=False, add_shadow=True, use_picture=True, hide_title=True, callback=self.finish)
...
class WellBeingApp(QWidget):
    ...
    def create_response_page(self):
        ...
        self.resp_cards_container.addWidget(yes_card)
        self.resp_cards_container.addWidget(no_card)
        layout.addLayout(self.resp_cards_container)

        self.back_btn_resp = QPushButton("Go Back")
        self.back_btn_resp.setFixedSize(150, 50)
        self.back_btn_resp.setStyleSheet("background-color: #BDBDBD; color: white; border-radius: 10px; font-weight: bold; border: none;")
    ...
    def create_styled_response_card(self, title, bg_color, btn_color, icon, callback):
        ...
        btn.setStyleSheet(f"background-color: {btn_color}; color: white; border-radius: 25px; font-size: 24px; font-weight: bold; border: none;")
